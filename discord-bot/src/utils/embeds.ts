import { EmbedBuilder, AttachmentBuilder, ActionRowBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ButtonBuilder, ButtonStyle } from 'discord.js';
...
export async function createFullScanEmbed(
  ...
  // Add Server ID field if available
  if (serverId) {
    try {
      pingEmbed.addFields({
        name: 'Server ID',
  ...
  if (!result.connection) {
    ...
    // Add dashboard button if URL and serverId are configured
    if (dashboardUrl && serverId) {
      components.push(createDashboardButton(dashboardUrl, serverId));
    }
...
/**
 * Create a dashboard button for viewing full results
 * Returns an ActionRow with a Link button to the dashboard
 */
export function createDashboardButton(
  ...
  const button = new ButtonBuilder()
    ...
    .setURL(`${dashboardUrl}/servers/${serverId}`)
    .setEmoji('📊');
