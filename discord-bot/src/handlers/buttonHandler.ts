...
import { RESET_CONFIRM_ID, RESET_CANCEL_ID } from '../commands/slash/reset.js';
...
export async function handleResetButton(interaction: ButtonInteraction): Promise<void> {
  ...
  // Handle cancel button - just dismiss the confirmation
  if (customId === RESET_CANCEL_ID) {
    await interaction.update({
      content: '❌ Reset cancelled. Your configuration remains unchanged.',
      components: []
    });
    logger.debug(`[ResetButton] Reset cancelled for guild ${interaction.guildId}`);
    return;
  }

  // Handle confirm button - delete the configuration
  if (customId === RESET_CONFIRM_ID) {
    const deleted = guildConfigService.deleteConfig(interaction.guildId);

    if (deleted) {
      await interaction.update({
        content: '✅ **ReconMC configuration has been reset.**\n\nUse `/setup` to configure the bot again.',
        components: []
      });
      logger.info(`[ResetButton] Configuration deleted for guild ${interaction.guildId}`);
    } else {
      await interaction.update({
        content: '⚠️ No configuration found to delete. It may have been already reset.',
        components: []
      });
      logger.warn(`[ResetButton] No config to delete for guild ${interaction.guildId}`);
    }
    return;
  }
