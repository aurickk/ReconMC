...
export const scanQueue = pgTable('scan_queue', {
  ...
  // Group by hostname + port (stable across DNS changes). Raw IP inputs have hostname=null
  // so they fall back to resolvedIp-based dedup at the application layer.
  uniqueEntry: unique('idx_scan_queue_unique').on(table.hostname, table.port),
}));
...
export const servers = pgTable('servers', {
  ...
  // Group by hostname + port (stable across DNS changes).
  // Raw IP inputs have hostname=null so they use resolvedIp-based dedup at the app layer.
  uniqueServer: unique('servers_unique').on(table.hostname, table.port),
}));
