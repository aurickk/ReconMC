...
export async function queueRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Params: { id: string }; Body: { reason?: string } }>(
    ...
    { onRequest: requireTrustedNetwork },
    async (request, reply) => {
      const { reason = 'Requeued: no sessions available' } = request.body ?? {};

      try {
        await requeueScan(db, request.params.id, reason);
        return reply.send({ message: 'Scan requeued successfully' });
      } catch (err) {
        fastify.log.error(err);
        return reply.code(500).send({ error: 'Failed to requeue scan', message: String(err) });
      }
    }
  );
